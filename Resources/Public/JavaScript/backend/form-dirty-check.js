 ...
 * and module switching, showing the standard "unsaved changes" modal dialog.
 */
...
class FormDirtyCheck {
    ...
    initialize() {
        ...
        // Detach consumer when module is unloaded
     ...
     * Returns a native Promise that resolves to allow navigation or stays pending to block it.
     *
     * Uses native Promise (not jQuery Deferred) to match TYPO3 core's FormEngine pattern
     * and ensure correct behavior with ConsumerScope's Promise.all() in invoke().
     */
    consume(interactionRequest) {
        ...
        return new Promise((resolve) => {
            this.showConfirmModal().then(() => resolve());
        });
    }
