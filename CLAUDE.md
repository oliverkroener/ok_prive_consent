...
TYPO3 CMS extension (`ok_prive_consent`) providing a backend module for managing Prive Consent banner scripts. Administrators edit JavaScript snippets via a Monaco Editor in the TYPO3 backend; the scripts are stored in the `sys_template` table and rendered on the frontend via TypoScript.
...
- **`DatabaseService`** – Queries/updates the custom `tx_ok_prive_consent_banner_script` field on `sys_template`. Also exposes `renderBannerScript()` as a TypoScript USER function for frontend output. Uses `SiteFinder` to resolve the site root page.

### Frontend Rendering

TypoScript in `Configuration/TypoScript/setup.typoscript` defines `lib.priveScript` (USER object calling `DatabaseService->renderBannerScript`), inserts the cookie button HTML and banner script via `page.footerData` (keyed by `crc32('ok_prive_consent')` to avoid collisions), and includes CSS via `page.includeCSS`.
